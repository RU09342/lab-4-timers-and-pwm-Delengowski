...
#define BUTTON1PRESSED !(P1IN & BIT3)
#define LED1_ON        P1OUT |= BIT0;                       //LED P1.0 ON
#define LED1_OFF       P1OUT &= ~BIT0;                      //LED P1.0 OFF
#define LED2_ON        P1OUT |= BIT6;                       //LED 1.6 ON
#define LED2_OFF       P1OUT &= ~BIT6;                      //LED 1.6 OFF
...
#define Dis_Int_But1   P1IE &= ~BIT3;                       // disable interrupts for P1.3 button
#define En_Int_But1    P1IE |= BIT3;                        //Enable Interrupts for P1.3 Button
#define ES_Neg_But1    P1IES &= ~BIT3;                      //TOGGLE INTERRUPT EDGE: LOW TO HIGH (button release)
#define ES_Pos_But1    P1IES |= BIT3;                       //TOGGLE INTERRUPT EDGE: HIGH TO LOW (BUTTON PRESS)
...
__interrupt void Timer1_ISR(void)
{
    ...
    case 1://GOING OFF -> OFF
        En_Int_But1;
        P1IFG &= ~BIT3;             //Clear Flag P1.3 button interrupt
        ES_Pos_But1;
        TimerA1_OFF;
...
void ButtonSetup()
{
    P1DIR &= ~BIT3;            //Set Button P1.3 to input

    P1REN |= BIT3;             //Enable Pull Up Resistor for P1.3 Button

    P1OUT |= BIT3;             //Active High Button P1.3

    P1IE |= BIT3;              //Enable Interrupts P1.3 Button

    P1IES |= BIT3;             //P1.3 Interrupt Triggers on Positive Edge(When pressing down)

    P1IFG &= ~BIT3;            //Clear P1.3 Interrupt Flag in case of immediate interrupt
}
